...
const SnippetCard: React.FC<SnippetCardProps> = ({ snippet, onUpdate, onDelete }) => {
      ...
      >
        <div className={`${bgColor} p-4 h-full flex flex-col`}>
          ...
          <div className="flex-grow overflow-hidden">
            <Highlight theme={themes.dracula} code={snippet.content} language={snippet.language as any}>
              {({ className, style, tokens, getLineProps, getTokenProps }) => (
                <pre className={`${className} text-sm`} style={{ ...style, background: 'transparent', margin: 0, padding: 0 }}>
          ...
          <div className="mt-2 flex justify-between items-center">
            <span className="text-sm text-gray-600">{formatDate(snippet.date)}</span>
            <div className="flex space-x-2">
              <button
                ...
                title="Copy link"
              >
                {isLinkCopied ? <Check size={16} className="text-green-500" /> : <LinkIcon size={16} />}
              </button>
              <button
                onClick={handleCopy}
                className="p-2 bg-gray-100 rounded-full shadow-md hover:bg-gray-200 transition-colors"
